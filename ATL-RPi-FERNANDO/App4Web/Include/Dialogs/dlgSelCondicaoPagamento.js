var dlgCondicaoPagamentoPaginacao = [];
//
//
// OBS:
//
...
//
//
function dlgCondicaoPagamentoCarrega(aDados) {
...
function dlgCondicaoPagamentoTabelaCriar(aTabela, aAcaoSelecao) {
	//
	// Montando a tabela da Janela de Pesquisa de Condicao de Pagamento: tab_dlgSelCondicaoPagamento
	//
	var tab_dlgEstrutura = [];
	tab_dlgEstrutura[0]  = new tabColuna('cAcao',       'string' , new tabColunaConteudo('t_0', '0',  '0', 'NNN', 'Acao',     "", null));
	tab_dlgEstrutura[1]  = new tabColuna('cCpgDescr',   'string' , new tabColunaConteudo('t_1', '1',  '1', 'NNN', 'Condição', "", null));
	tab_dlgEstrutura[2]  = new tabColuna('cCpgCod',     'string' , new tabColunaConteudo('t_2', '2',  '2', 'NNN', 'Codigo'  , "", null));
	tab_dlgEstrutura[3]  = new tabColuna('cCpgParcelas','string' , new tabColunaConteudo('t_3', '3',  '3', 'NNN', 'Parcelas', "", null));
	...
	var aCustomD = [];
	aCustomD[0] = new tabBotoesCustom("CUS", aAcaoSelecao, "btnCpgSelect", "Seleciona", "BTN", "Seleciona Cond.Pagameto", "verde fa fa-thumb-tack cssHand");

	// Variavel global no Javascript
	aTabRowButtons[aTabela]["CUS"] = aCustomD;

	// Exibindo a pagina corrente da pesquisa no Dialog
	dlgCondicaoPagamentoTabelaMostraPagina();

}

function dlgCondicaoPagamentoTabelaMostraPagina() {
	   var pagina = 0;
	   var pagina_ultima = 0;
	   if ( array_key_exists("pagina",dlgCondicaoPagamentoPaginacao) == true ) {
		  pagina = dlgCondicaoPagamentoPaginacao["pagina"];
		  pagina_ultima = dlgCondicaoPagamentoPaginacao["pagina_ultima"];
	     
    } 
	   $("#valDlgCpgPagina").html(pagina).show();
	   $("#valDlgCpgUltimaPagina").html(pagina_ultima).show();
}

function dlgCondicaoPagamentoShowLoader() {
	  document.getElementById("tab_cpg_loader").style.display = "block";
	  document.getElementById("tab_cpg_loader_texto").style.display = "block";
 }
 
 function dlgCondicaoPagamentoHideLoader() {
	  document.getElementById("tab_cpg_loader").style.display = "none";
	  document.getElementById("tab_cpg_loader_texto").style.display = "none";
 }       
	   
 function dlgCondicaoPagamentoAcao(aAcao,aComple) {
 	   
     // Processar, Paginacao e Dialog conforme solicitado via combobox
     aAcao = aAcao.toUpperCase();
     aComple = aComple.toUpperCase();
     dlgCondicaoPagamentoProcessarDados(aAcao, aComple);

 }       

 function dlgCondicaoPagamentoReset(aOnde) {
	 ...
	 if ( aOnde == "natureza_parcelas") {
	    $('#fld_cpg_parcelas').empty().append('<option>Carregar</option>');    
	 }
}
 
 function dlgCondicaoPagamentoLimpar() {

	 dlgCondicaoPagamentoLimpar("");
	 
     // Inicializa os Criterios
     $("#fld_ncp_id").val(0).change();
     $("#fld_ncp_grupo").val(0).change();
     $("#fld_cpg_parcelas").val(0).change();
 ...
 function dlgCondicaoPagamentoProcessarDados(aAcao, aComple) {
	 ...
	 //Aciona o AJAX e indica como receber o JSON
      ...
      if ( aAcao == "SEL_CONDICAO" ) {
    	  ...
    	  dlgCondicaoPagamentoShowLoader();

    	  dlgCondicaoPagamentoDesativaBotoesPaginacao();  		    
      }
	  
      var url = js_link + "dlgCondicaoPagamento_AJAX/ajax/index/acao/" + aAcao
                        + "/complemento/" + aComple
                        + "/empresa/" + empresa
                        + "/filial/" + filial 
                        + "/usuario/" + usuario
      ...
      // Se tiver carregada
      if ( array_key_exists("pagina", dlgCondicaoPagamentoPaginacao ) == true ) {
   		 url += "/pagina/" + dlgCondicaoPagamentoPaginacao["pagina"]
              + "/pagina_corrente/" + dlgCondicaoPagamentoPaginacao["pagina_corrente"]
   	          + "/pagina_primeira/" + dlgCondicaoPagamentoPaginacao["pagina_primeira"]
   	          + "/pagina_anterior/" +  dlgCondicaoPagamentoPaginacao["pagina_anterior"]
              + "/pagina_proxima/" + dlgCondicaoPagamentoPaginacao["pagina_proxima"] 
              + "/pagina_ultima/" + dlgCondicaoPagamentoPaginacao["pagina_ultima"]  
              + "/pagina_linhas/" + dlgCondicaoPagamentoPaginacao["pagina_linhas"]  
              + "/pagina_offset/" + dlgCondicaoPagamentoPaginacao["pagina_offset"]
   	          + "/pagina_total_linhas/" + dlgCondicaoPagamentoPaginacao["pagina_total_linhas"]    
              + "/pagina_acao/" + aComple;                  
    }
	  ...
	  $.getJSON(url, function ( dados_retorno ){ 
	     ...
	     dlgCondicaoPagamentoProcessarRetornoAJAX( dados_retorno);   			   
	  } ); 
	              
 }   

 function dlgCondicaoPagamentoProcessarRetornoAJAX( aDados ) {
	 var aAcao  = aDados.acao;
     var aTabId = "tab_dlgSelCondicaoPagamento";
     ...
     aTabAtivaUserCellFunction[aTabId] = false;           
 	 ...
 	 if (aDados.dados.CPG != null ){	
	         ...
	         tabFootInsert(aTabId);
	   		 ...
	   		 var lCondicaoPagamento = aDados.dados.CPG.length;
	   		 ...
	   		 for ( var l=0; l < lCondicaoPagamento; l++) {		   	   
	            ...
	            var c=0;
		   	    ...
		   	    _cpg_id = aDados.dados.CPG[l].referencia.CPG.cpg_id;
		   	    rId = _cpg_id;
                ...
                // Coluna 0 - botoes
	   		    ...
	   		    cId = rId + "_e"+c;
	   		    linha[c++] = new tabColunaConteudo(cId, "", "", "NONE", aDados.dados.CPG[l].referencia.CPG.cpg_descr, sitLinha, null);
	   		    
	   	        // Coluna 2 - Codigo
	   		    cId = rId + "_e"+c;
	   		    linha[c++] = new tabColunaConteudo(cId, "", "", "NONE", aDados.dados.CPG[l].referencia.CPG.cpg_cod, sitLinha, null);
   		    
	   	        // Coluna 3- Quantidade de Parcelas
	   		    cId = rId + "_e"+c;
	   		    linha[c++] = new tabColunaConteudo(cId, "", "", "NONE", aDados.dados.CPG[l].referencia.CPG.cpg_parcelas, sitLinha, null);
	   		 ...
	   		 dlgCondicaoPagamentoPaginacao["pagina"]              = aDados.paginacao.pagina; // Pagina corrente
	         dlgCondicaoPagamentoPaginacao["pagina_corrente"]     = aDados.paginacao.pagina_corrente;
	         dlgCondicaoPagamentoPaginacao["pagina_primeira"]     = aDados.paginacao.pagina_primeira;
	         dlgCondicaoPagamentoPaginacao["pagina_anterior"]     = aDados.paginacao.pagina_anterior;
	         dlgCondicaoPagamentoPaginacao["pagina_proxima"]      = aDados.paginacao.pagina_proxima;
	         dlgCondicaoPagamentoPaginacao["pagina_ultima"]       = aDados.paginacao.pagina_ultima;
	         dlgCondicaoPagamentoPaginacao["pagina_linhas"]       = aDados.paginacao.pagina_linhas;
	         dlgCondicaoPagamentoPaginacao["pagina_offset"]       = aDados.paginacao.pagina_offset;
	         dlgCondicaoPagamentoPaginacao["pagina_total_linhas"] = aDados.paginacao.pagina_total_linhas;
	         dlgCondicaoPagamentoPaginacao["pagina_acao"]         = aDados.paginacao.pagina_acao;

	 	     // Mostrando a pagina como ficou, se achou ou nao
	     	 dlgCondicaoPagamentoTabelaMostraPagina();
	     	 
	     	 // Verifica se precisa ativar botoes
	         dlgCondicaoPagamentoAtivaBotoesPaginacao();
	          	         
		  } else {
			  dlgCondicaoPagamentoDesativaBotoesPaginacao();
			 
			 dlgCondicaoPagamentoPaginacao = []; // inicializa array
 	  ...
 	  var out = setTimeout(dlgCondicaoPagamentoHideLoader,500);
 	         	  
  }
 
 function dlgCondicaoPagamentoAtivaBotoesPaginacao() {

	   // Apenas ativa os botoes se tiver mais de uma pagina
	   _tot = parseInt($("#valDlgCpgUltimaPagina").html());
	   if ( isNaN(_tot) ) { _tot = 0;}
	   
	   if ( _tot > 1 ) {
		   $("#bdCpgPrimeira").prop("disabled", false);
		   $("#bdCpgProxima").prop("disabled", false);
		   $("#bdCpgAnterior").prop("disabled", false);
		   $("#bdCpgUltima").prop("disabled", false);
		   
		   $("#bdCpgPrimeira").removeClass("disabled");
		   $("#bdCpgProxima").removeClass("disabled");
		   $("#bdCpgAnterior").removeClass("disabled");
		   $("#bdCpgUltima").removeClass("disabled");
	   }
 }
 
 function dlgCondicaoPagamentoDesativaBotoesPaginacao() {
	   $("#bdCpgPrimeira").prop("disabled", true);
	   $("#bdCpgProxima").prop("disabled", true);
	   $("#bdCpgAnterior").prop("disabled", true);
	   $("#bdCpgUltima").prop("disabled", true);

	   $("#bdCpgPrimeira").addClass("disabled");
	   $("#bdCpgProxima").addClass("disabled");
	   $("#bdCpgAnterior").addClass("disabled");
	   $("#bdCpgUltima").addClass("disabled");
 }