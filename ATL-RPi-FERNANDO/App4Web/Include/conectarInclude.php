...
function Conectar(){
	...
	$_db_server = $start->_session->selectSession("DOM_DB_SERVER");
	$_db_name   = $start->_session->selectSession("DOM_DB_NAME");
	$_db_login  = $start->_session->selectSession("DOM_DB_USER");
	$_db_passwd = $start->_session->selectSession("DOM_DB_PASSWD");
	...
	if ( $_db_server == null or empty($_db_server) ) {
		...
		try{
			...
			echo 'Conectar.php: Erro: '.$e->getMessage();
			return null;
		}
	}
}
