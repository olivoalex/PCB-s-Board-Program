...
//------------------------------------------------------------------------------
        ...
        var _loadOnHashChange = function(){
            $(window).on('hashchange.lobiAdmin', function(ev) {
				...
				//alert("_loadHashChange Hash: " + hash + " Text: "+text);
                me.load(hash, text);
            });
        };
        var _rewriteUrl = function(url){
			...
			// AFV - adaptacao#1
			// Se tiver um .php, .phtml, .html no HASH nao faz nada
			var tem = url.indexOf(".php");
			if ( tem == -1 ) {
			   tem = url.indexOf(".phtml");
			   if ( tem == -1 ) {
			      tem = url.indexOf(".html");
				  if ( tem != -1 ) {
					 // retorna esse .html
					 return "HTML/" + url;
				  }
			...
			//
			// AFV - adaptacao#2
			// Antes vou ver se existe alguma variavel QUERYSTRING na URL
			tem = url.indexOf("?");
			var _complemento = tem == -1 ? url.split("\\") : url.split("?");
			if ( tem == -1 ) {
			   tem = url.indexOf("/");
			   _complemento = tem == -1 ? url.split("\\") : url.split("/");
			} else {
			   _complemento = url.split("?");
			}
						
			var url_complemento = "";
			if (_complemento) {
				if ( _complemento.length > 1 ) {
				   url = _complemento[0];	
				   for ( var i=1; i < _complemento.length; i++ ) {
					  url_complemento += _complemento[i].toString();
				   }
				}
			} 	

            //alert("Complemento: " + url_complemento);			
			
            if ( ! CONFIG.enableUrlRouting){
				    //alert("Enable ?: " + url + " Comple: " + url_complemento);			
                    return url;
            }
            if (LobiAdminRoutes[url]){
				//alert("Routes/url: " + url + " Route: " + LobiAdminRoutes[url]);	
...
//                        window.console.log(route);
                        if ( vars == ".php" ) {
						  route = route.replace('$'+replaceVarIndex, url);
						} else {
						  if ( vars == ".phtml" ) {
							route = route.replace('$'+replaceVarIndex, url);  							 
						  } else {
						    // Paginas html sem criterio para validar url/hash
					...
					// AFV
					// Verificando se precisa adicionar o complemento
					//if ( url_complemento ) {
					//	route += "?"+url_complemento;
					//}
					
					//alert("retornando route: "+route);
                    return route;
                }
            }
			
			//alert("retornando: "+url);
            return url;
        };
//------------------------------------------------------------------------------
//----------------PROTOTYPE FUNCTIONS-------------------------------------------
//------------------------------------------------------------------------------
        ...
        this.load = function (url, title, isError) {
			...
			// AFV
			// garantir que essa function exista, deve estar na portal.phtml
			refresh_inicializa();
			...
			var _tem = _hash.indexOf("?");
			if ( _tem == -1 ) {
			   // Por barra normal ou invertida
			   _tem = _hash.indexOf("/");
			   _complemento = _tem == -1 ? _hash.split("\\") : _hash.split("/");
			} else {
			   // Por query string usando ?
			   _complemento = _hash.split("?");
			}
						
		    // Determinando como executar, via url ou criar um caminho
			// De inicio vamos pela url recebida
			var _executar = "";			
			// Modo de execucao, via URL (false) ou via programa/menu/portal (true)
			var _executar_portal = false;
			// Para montar os parametros ou complemento da url (query_string)
			var _url_complemento = "";			
			// Foi splitado
			if (_complemento) {
				if ( _complemento.length > 1 ) {
				   _executar = _complemento[0];	
				   _executar_portal = true;
				   for ( var i=1; i < _complemento.length; i++ ) {
					  _url_complemento += "/" + _complemento[i].toString();
				   }
				}
			} 	
			
			if (_executar_portal == true ) {
			   // tem que montar a URL para o padrao do portal
			   // Ex. http://11.12.13.30/App4Web/Portal/stkm000300/index/index\mnu_id\100\opc_id\200\prg_id\300
			   _executar = jsMontaUrl("index", "menu", _executar.toString(), _url_complemento); 
			} else {
			   // Nao faz nada a URL eh a q recebemos por parametro
			   _executar = jsMontaUrlDireta(url);			
			}
			
			//alert("Portal ? "+ _executar_portal + " hash: "+ _hash + "\nLoading Ajax: "+ url+ "\n _executar: " + _executar+ "\nComplemento: "+ _url_complemento + "\n Titulo: " + title +"\n Error: "+ isError); 
			// Loading Ajax: pages/stkm000200.html?mnu_id\100\opc_id\200\prg_id\200 Titulo: undefined Error: undefined
            ...
            $.ajax({
                ...
                success: function (html) {
                    ...
                    me.$currentPage = url;
					if (  me.$breadCrumbs == null) { // afv
                       me.drawBreadcrumbs();
					} else {
					   me.forceBreadcrumbs(); //afv
					}
                    //set the page title
					title = "Painel | Agrotechlink&reg;";
                    if (title) {
                        $('title').html(title);
                    }
        ...
        this.loadController = function (aController, aTask, aAction, aParams) {            
            //get the panel in which we are loading page
					
 		    // tem que montar a URL para o padrao do portal
			// Ex. http://11.12.13.30/App4Web/Portal/stkm000300/index/index\mnu_id\100\opc_id\200\prg_id\300
			_executar = jsMontaUrl(aController, aTask, aAction, aParams); 
			_triggerEvent('beforePageLoad', _executar);
			
			//alert("loadController : "+ _executar);
            ...
            $.ajax({
                ...
                success: function (html) {
                    contentPanel.html(html)[0].stopLoading();
                    _triggerEvent('pageLoaded', _executar);
                },
                error: function (xhr, status, code) {
                    if (!isError) {
                        if (xhr.status === 404) {
        ...
        this.reload = function(){
			//alert("Pagina Corrente is " + me.$currentPage)
		...
		// afv
		// Forcando breadcrumbs manualmente
		this.setBreadcrumbs = function (aOptions) {
			me.$breadCrumbs = aOptions;
		}
		
        this.forceBreadcrumbs = function(){
            var $br = $(CONFIG.breadcrumbsListSelector);
			var aOptions = me.$breadCrumbs;
            $br.html("");
			
			// Apenas se nao for nula
			if ( aOptions ) {
				for (var i = 0; i<aOptions.length; i++){
					var $li;
					var path = aOptions[i];
					if (i === 0){
						var $li = $('<li>'+path["texto"]+'</li>');
						$li.addClass('active');
						if (path["cssicon"] != "NONE" ) {
							$li.prepend("<i class='"+path["cssicon"]+"'></i>", " ");
						}
					}else{
						var hash_opc = path["href"];
						if ( hash_opc == "NONE" ) {
							hash_opc = "#";
						}
						var $li = $('<li><a href="'+hash_opc+'">' + path["text"] + '</a></li>');
						if (path["cssicon"] != "NONE") {
							$li.prepend("<i class='"+path["cssicon"]+"'></i>", " ");
						}
					}
				...
				// Limpa breadcrumbs para a proxima chamada
				 me.$breadCrumbs=null;
			}
            return me;
        };		
