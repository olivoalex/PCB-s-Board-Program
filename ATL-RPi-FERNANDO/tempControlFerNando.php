...
echo "versao: 0 - revisao: 28052017\n";
echo "{$setSPACER}";
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
...
$gpio_17on  = shell_exec("/usr/local/bin/gpio -g write 17 1\n");
	echo "{$setSPACER}";
	echo "GPIO_17 - FAN CONTROLL - LIGADO!\n T = " . $tempCPU / 1000 . " *C\n";		
	echo "TEMP CPU MAIOR QUE TEMP CONTROLE!\n";}
else if($tempCPU < $Tfan_controle){
$gpio_17off = shell_exec("/usr/local/bin/gpio -g write 17 0\n");
	echo "{$setSPACER}";
	echo "GPIO_17 - FAN CONTROLL - DESLIGADO!\n T = " . $tempCPU / 1000 . " *C\n";	
